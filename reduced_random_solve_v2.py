...
class Board:
	...
	def Sys_Generator(self, net_grid, blocks, steps):

		# Applying Method 2:
		all_block_pos = []
		for i in range(len(net_grid)):
			for j in range(len(net_grid[0])):
...
for i in range(500000):
	G = Game('mad_1.bff')
	G.database()
	print(G.lazor_start, G.lazor_path)
	B = Board(G.grid,G.lazor_start, G.lazor_path,G.pointer)

	mesh_board = B.sample_board(B.sampler(G.grid), G.blocks, G.grid)

	mesh = B.Sys_Generator(mesh_board, G.blocks, 500000)
